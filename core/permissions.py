...
class PlaceOwnerOrReadOnly(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        # Allow GET, HEAD, OPTIONS requests for any user (authenticated or not for public APIs,
        # or rely on IsAuthenticated if that's a global default or added to views).
        if request.method in permissions.SAFE_METHODS:
            return True
