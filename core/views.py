...
from core.utils import (
    get_printer_sn_for_item,
    get_print_content,
    api_print_request,
    update_last_ordering_time,
    handle_lunch_dinner_time,
    translate_menu_name_description,
)
from qrmenu_backend.settings import user as USER_NAME
from qrmenu_backend.settings import user_key as USER_KEY
...
def create_order_intent(request):
    try:
        if not request.body:
            return JsonResponse(
                {"success": False, "error": "Request body is empty."}, status=400
            )
        ...
        if place_id is None:
            return JsonResponse(
                {"success": False, "error": "Missing 'place' key in request data."},
                status=400,
            )
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "error": "Place not found."}, status=404
            )

        today = timezone.now().date()
        max_daily_id_result = models.Order.objects.filter(
            place_id=place_id, created_at__date=today
        ).aggregate(Max("daily_id"))
        ...
        for item_data in order_items_data_from_request:
            ...
            if quantity <= 0:
                return JsonResponse(
                    {
                        "success": False,
                        "error": f"Quantity for item id {menu_item_id_str} must be positive.",
                    },
                    status=400,
                )

            try:
                menu_item_obj = models.MenuItem.objects.get(
                    id=menu_item_id_str, place_id=place_id
                )
            except models.MenuItem.DoesNotExist:
                return JsonResponse(
                    {
                        "success": False,
                        "error": f"MenuItem with id {menu_item_id_str} not found for this place.",
                    },
                    status=404,
                )
            ...
            processed_order_item_details_for_json.append(
                {
                    ...
                    "name_to_print": menu_item_obj.name_to_print
                    or menu_item_obj.name,  # Added name_to_print
        ...
        if validated_items_for_order:
            ...
            main_sn_id = get_printer_sn_for_item(
                printers_for_place, first_item_menu_obj
        ...
        with transaction.atomic():
            ...
            for item_to_create in validated_items_for_order:
                models.OrderItem.objects.create(
                    ...
                    category_name_at_time_of_order=item_to_create[
            ...
            for item_instance in order.items.all():
                ...
                sn_id = get_printer_sn_for_item(
                    printers_for_place, menu_item_obj
                )  # Uses correct function name
                if sn_id:
                    name_to_print_for_item = (
                        menu_item_obj.name_to_print
                        if menu_item_obj.name_to_print
                        else menu_item_obj.name
                    )
            ...
            if grouped_items_for_printing:
                ...
                for sn_id, items_for_this_printer in grouped_items_for_printing.items():
                    print_content = get_print_content(
                        order.daily_id,
                        data,
                        items_for_this_printer,
                        "B1",
                        datetime_to_print_str,
                    )
                    try:
                        print_response = api_print_request(
                            USER_NAME, USER_KEY, sn_id, print_content
                        )
                        # Corrected Xpyun success check: expects 'code': 0 and 'msg': 'ok'
                        if not (
                            print_response.get("code") == 0
                            and print_response.get("msg") == "ok"
                        ):
                            print(
                                f"Warning: Print job to SN {sn_id} failed or had unexpected response. Response: {print_response}"
                            )
                            print_jobs_successful = False
        ...
        return JsonResponse(
            {"success": False, "error": f"Missing key in request data: {str(e)}"},
            status=400,
        )
...
def create_category_intent(request):
    try:
        ...
        category = models.Category.objects.create(
            place_id=data["place"], name=data["name"], name_en=name_en, name_pt=name_pt
        )
...
def create_menu_items_intent(request):
    if request.method == "POST":
        try:
            ...
            if not place_id_str:
                return JsonResponse(
                    {"success": False, "error": "Place ID is required."}, status=400
                )
            ...
            if not menu_item_name:
                return JsonResponse(
                    {
                        "success": False,
                        "error": "Menu item name is required and cannot be empty.",
                    },
                    status=400,
                )
            ...
            lunch_time_start, lunch_time_end, dinne_time_start, dinne_time_end = (
                handle_lunch_dinner_time(place, helper_data)
            )
            ...
            raw_name_en, raw_name_pt, raw_description_en, raw_description_pt = (
                translate_menu_name_description(form_data)
            )

            name_en = raw_name_en if raw_name_en is not None else ""
            name_pt = raw_name_pt if raw_name_pt is not None else ""
            description_en = (
                raw_description_en if raw_description_en is not None else ""
            )
            description_pt = (
                raw_description_pt if raw_description_pt is not None else ""
            )
            ...
            return JsonResponse(
                {"success": False, "error": f"Missing data: {str(e)}"}, status=400
            )
        except Exception as e:
            return JsonResponse(
                {
...
class ReorderCategoriesView(views.APIView):
    ...
    def post(self, request, place_id):
        ...
        if not isinstance(ordered_category_ids, list):
            return Response(
                {"error": "ordered_category_ids must be a list."},
        ...
        try:
            with transaction.atomic():
                existing_categories = {
                    cat.id: cat for cat in models.Category.objects.filter(place=place)
                }

                if len(ordered_category_ids) != len(existing_categories):
                    return Response(
                        {
                            "error": "The number of provided category IDs does not match the number of categories for this place."
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                for i, category_id in enumerate(ordered_category_ids):
                    if category_id not in existing_categories:
                        return Response(
                            {
                                "error": f"Category with ID {category_id} not found for this place or is invalid."
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
...
class ReorderMenuItemsView(views.APIView):
    ...
    def post(self, request, category_id):
        ...
        if not isinstance(ordered_item_ids, list):
            return Response(
                {"error": "ordered_item_ids must be a list."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            with transaction.atomic():
                existing_items = {
                    item.id: item
                    for item in models.MenuItem.objects.filter(category=category)
                }

                if len(ordered_item_ids) != len(existing_items):
                    return Response(
                        {
                            "error": "The number of provided item IDs does not match the number of items in this category."
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                for i, item_id in enumerate(ordered_item_ids):
                    if item_id not in existing_items:
                        return Response(
                            {
                                "error": f"Menu item with ID {item_id} not found in this category or is invalid."
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
            ...
            return Response(
                {"success": "Menu items reordered successfully."},
                status=status.HTTP_200_OK,
            )

        except Exception as e:
            return Response(
                {"error": f"An error occurred while reordering menu items: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
def reprint_order(request):
    try:
        ...
        items_to_reprint = (
            json.loads(order_detail_str)
            if isinstance(order_detail_str, str)
            else order_detail_str
        )
        ...
        for item_detail in items_to_reprint:
            ...
            try:
                menu_item_obj = models.MenuItem.objects.get(
                    id=menu_item_id, place_id=place_id
                )
                sn_id = get_printer_sn_for_item(
                    printers, menu_item_obj
                )  # Uses correct function name
                if sn_id:
                    name_to_print = (
                        menu_item_obj.name_to_print
                ...
                print(
                    f"Warning: MenuItem {menu_item_id} not found during reprint, skipping."
                )
                continue

        order_created_at_str = data.get(
            "order_created_at", timezone.now().strftime("%Y-%m-%d %H:%M:%S")
        )

        for sn_id, items_for_this_printer in grouped_for_reprint_by_sn.items():
            print_content = get_print_content(
                daily_order_id, data, items_for_this_printer, "B1", order_created_at_str
            )
            api_print_request(USER_NAME, USER_KEY, sn_id, print_content)
